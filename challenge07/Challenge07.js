...
const objLat = (obj) => {
    ...
    let formattedText = ("my name is " + obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1) + ' '+ obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1) + ' I am ' + obj.age + ' YO, and I love ' + obj.hobby + '.');
...
let fullName;
    ...
    for(let i=0;i<cvArr.length;i++){
        ...
        if(cvArr[i].firstName != null && cvArr[i].lastName != null ){
            fullName = cvArr[i].firstName + ' ' + cvArr[i].lastName;
            cvArr[i].fullName = fullName;
        }
        else if (cvArr[i].firstName == null){
           fullName=cvArr[i].lastName
           cvArr[i].fullName=fullName

        }
        else if (cvArr[i].lastName==null){
            fullName=cvArr[i].firstName
           cvArr[i].fullName=fullName 
        }
            delete cvArr[i].firstName;
            delete cvArr[i].lastName;
            delete cvArr[i].yearsOfExperience;
        }
        return cvArr;
